...
def move_snake():
    #move each snake part (starting from the back) to where the one in front of it is
...
def snake_hit_itself():
    #check one snake part at a time, to see if the front has hit that snake part
    for x in range(1,len(snake_grid_x)):   
        if snake_grid_x[0] == snake_grid_x[x] and snake_grid_y[0] == snake_grid_y[x]:
            return True
...
while running:
    ...
    #if the snake eats the dot, create a new dot location, increase your score, and grow the snake
